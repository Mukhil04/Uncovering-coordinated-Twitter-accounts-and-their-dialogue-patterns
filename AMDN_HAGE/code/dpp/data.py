...
class Batch():
    def __init__(self, in_time, out_time, length, index=None, in_mark=None, out_mark=None, in_tweet_type=None, out_tweet_type=None):
        ...
        self.in_tweet_type = in_tweet_type.long()
        self.out_tweet_type = out_tweet_type.long()
...
def load_dataset_from_split(data, split, log_mode=True):
    ...
    arrival_times = data_split['marks']
    ...
    tweet_type = data_split['tweet_type']
    num_classes = data['dim_process'] if marks is not None else 1
    return SequenceDataset(delta_times, marks=marks, num_classes=num_classes, log_mode=log_mode, tweet_type = tweet_type)
...
def collate(batch):
    ...
    in_tweet_types = [item[4] for item in batch]
    out_tweet_types = [item[5] for item in batch]
    index = torch.tensor([item[6] for item in batch])
    ...
    in_tweet_type = torch.nn.utils.rnn.pad_sequence(in_tweet_types, batch_first=True)
    out_tweet_type = torch.nn.utils.rnn.pad_sequence(out_tweet_types, batch_first=True)
    in_time[:,0] = 0 # set first to zeros
    print("start")
    print(len(in_tweet_types))
    print(len(in_tweet_types[0]))
    print("mid")
    print(len(in_marks))
    print(len(in_marks[0]))
    print("second mid")
    print(in_mark.size())
    print("hello")
    print(in_tweet_type.size())
    print("see")
    print(in_tweet_type)
...
class SequenceDataset(data_utils.Dataset):
    ...
    def __init__(self, delta_times=None, marks=None, in_times=None, out_times=None,
                 in_marks=None, out_marks=None, index=None, log_mode=True, num_classes=1, tweet_type=None):      
        ...
        if tweet_type is not None:
            self.in_tweet_type = [torch.Tensor(type[:-1]) for type in tweet_type]
            self.out_tweet_type = [torch.Tensor(type[1:]) for type in tweet_type]
        ...
        self.break_down_long_sequences(128)
        if log_mode:
    ...
    def break_down_long_sequences(self, max_seq_len):
        ...
        for idx in range(self.num_series):
            ...
            current_in_tweet_type = self.in_tweet_type[idx]
            current_out_tweet_type = self.out_tweet_type[idx]
            ...
            for b in range(num_batches):
                ...
                new_in_tweet_type = current_in_tweet_type[b * max_seq_len : (b + 1) * max_seq_len]
                new_in_tweet_types.append(new_in_tweet_type)
                new_out_tweet_type = current_out_tweet_type[b * max_seq_len : (b + 1) * max_seq_len]
                new_out_tweet_types.append(new_out_tweet_type)
        ...
        self.in_tweet_types = new_in_tweet_types
        self.out_tweet_types = new_out_tweet_types
    ...
    def __getitem__(self, key):
        return self.in_times[key], self.out_times[key], self.in_marks[key], self.out_marks[key], self.in_tweet_types[key], self.out_tweet_types[key], self.index[key]
