...
class AttentiveLayer(BaseModule):
    ...
    def forward(self, input, gmm_prob = None):
        ...
        if self.pos_enc_config:  # positional encoding 
            ...
            print(input.in_tweet_type)
            print("break")
            print(len(input.in_mark))
            print(len(input.in_mark[0]))
            print(input.in_mark)
            print("come on")
            result = input.in_tweet_type.unsqueeze(-1)
            # tweet_type = input.in_tweet_type[]
            if not self.add:
                if self.use_community:
                    ...
                    x = torch.cat([mark_tokens, time_embedding, positions, result], dim=2)
