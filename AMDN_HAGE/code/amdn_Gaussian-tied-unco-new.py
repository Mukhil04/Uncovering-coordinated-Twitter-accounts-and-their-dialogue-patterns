...
def load_data(args):
    ...
    for split in ['train', 'dev', 'test']: 
        ...
        seq_dataset.append(dpp.data.load_dataset_from_split(data, split, log_mode=False)) # SequenceDataset, log change here
    d_train, d_val, d_test = seq_dataset
    # d_train = seq_dataset[0]
    ...
    if args.decoder_name in ['RMTPP', 'FullyNeuralNet', 'Exponential']:
        ...
        mean_out_train, std_out_train = d_train.get_mean_std_out() # log change here
...
def create_model(num_classes, num_sequences, args, mean_out_train, std_out_train):
    ...
    model = BalancedDataParallel(args.gpu0sz,model)
    model = model.to(args.device)
    #opt = torch.optim.Adam(model.parameters(), weight_decay=args.regularization, lr=args.learning_rate)
